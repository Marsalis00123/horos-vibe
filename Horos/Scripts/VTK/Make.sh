...
elif [ -d "/opt/homebrew/opt/libtiff/include" ]; then
    rsync /opt/homebrew/opt/libtiff/include/tiff*.h "$install_dir/include/vtktiff/libtiff/"
fi

# wrap the libs into one
mkdir -p "$install_dir/wlib"
ars=$(find "$install_dir/lib" -name '*.a' -type f)
for lib in /opt/homebrew/opt/libtiff/lib/libtiff.a /opt/homebrew/opt/libpng/lib/libpng.a; do
    [ -f "$lib" ] && ars="$ars $lib"
done
